...
export function buildGenerateSystem(tonePrompt: string): string {
  return [
    ...
    '- comentariosDoEditor (quando houver): são observações de quem narrou a partida AO VIVO —',
    '  use como cor/contexto e incorpore ao texto com naturalidade (não cite "o editor"). Trate',
    '  como fato, mas NÃO invente estatística/número a partir delas: o que não for placar/dado',
    '  estruturado é leitura de jogo, escreva como tal.',
    '- narracaoEspn (quando houver): a NARRAÇÃO LANCE A LANCE da partida, em INGLÊS, cada linha',
    '  prefixada pelo minuto e (quando houver) pelo time entre colchetes. É a sua MATÉRIA-PRIMA',
    '  principal para CONTAR O JOGO: use-a para reconstruir a história em ordem cronológica',
    '  (como o jogo começou, os lances de perigo, defesas, faltas, viradas de pressão, o fechamento)',
    '  e dar textura ao body — NÃO é só uma lista de eventos. TRADUZA para português natural',
    '  (não transcreva o inglês nem traduza ao pé da letra).',
    '  RECONCILIAÇÃO — os blocos ESTRUTURADOS mandam: placar/gols/cartoes/substituicoes/estatisticas',
    '  são a verdade sobre QUEM, QUANDO e QUANTO; a narracaoEspn detalha e colore esses fatos, e NÃO',
    '  os contradiz. Para grafia de nome, prefira o nome como aparece em gols/escalacoes (a narração',
    '  pode vir sem acento ou em inglês). Se a narração divergir do placar/gol estruturado, siga o',
    '  estruturado e ignore a divergência.',
    '  NÃO invente número/estatística que não esteja nos fatos, NÃO deduza consequências (eliminação,',
    '  próximo adversário), e NÃO cite a fonte ("segundo a ESPN", "na narração"): escreva como',
    '  reportagem própria, na voz editorial.',
    '',
    ...
    '- metaTitle: o <title> da página. ≤60 caracteres, com a PALAVRA-CHAVE no começo. DEVE',
    '  manter a IDENTIDADE do title — o gancho específico (quem/o quê decidiu, o nome',
    '  próprio que importa), só mais enxuto. NUNCA uma versão genérica do assunto. Pode',
    '  ser mais seco que o title, mas sem clickbait e sem frases vagas ("veja", "confira").',
    ...
    '- metaTitle: comece com "Time A N x M Time B" e, no espaço restante (≤60 chars), inclua',
    '  o GANCHO que decidiu o jogo (quem marcou/decidiu) — a mesma identidade do title, só',
    '  mais enxuta. Ex.: "Espanha 4 x 0 Arábia Saudita: Oyarzabal marca 2 na Copa". O PLACAR',
    '  nunca pode faltar; PROIBIDO genérico ("veja o resultado", "confira como foi") e',
    '  PROIBIDO reduzir só a "placar + competição" quando há um gancho claro no fato.',
    '',
...
export const GENERATE_SCHEMA = {
  ...
  properties: {
    ...
    metaTitle: { type: 'string', description: '<title> SEO, ≤60 chars, palavra-chave no começo, mantendo o gancho/identidade do title (nunca genérico)' },
