...
async function main() {
  ...
  const svc = new MatchSummaryService(prisma, new EspnService(), new EventsService());
  ...
  const matches = await prisma.match.findMany({
    ...
    select: { id: true, externalIds: true, homeTeam: { select: { shortName: true } }, awayTeam: { select: { shortName: true } } },
  ...
  for (const [i, m] of matches.entries()) {
    try {
      ...
      console.warn(`  [${i + 1}/${matches.length}] failed: ${(e as Error).message.split('\n')[0]}`);
    }
    await sleep(120);
  }
  console.log(`done — ${withLineup} with lineup, ${empty} without (no ESPN lineup published)`);
