...
// R2, for mirroring the competition crest (same convention as seed-clubs /
// import-competition-logos.mjs). PUBLIC must be the CDN base (cdn.cravei.app) —
// in prod that's STORAGE_PUBLIC_BASE_URL; from a dev .env, pass it explicitly.
...
/**
 * Mirror ESPN's league logo (light + dark) into R2 at competitions/<slug>/logo*.png
 * and return the public URLs. Idempotent: re-runs overwrite the same keys. Returns
 * nulls if storage isn't configured or ESPN has no logo, so the seed still works.
 */
...
async function run(): Promise<void> {
  ...
  const competition = await prisma.competition.upsert({
    ...
    // Only fill the logo if we resolved one (don't clobber an existing crest with null).
  ...
  // Período = min/max das datas dos jogos que semeamos (truncado ao dia, UTC).
  // Estende sozinho quando quartas/semis/final ganharem data num re-seed.
  ...
  // Knockout rounds — ALL phases up front (preliminares..Final) so a chave mostra o
  // caminho inteiro. Atualiza ordem/legs em re-run (as preliminares empurraram as
  // oitavas de 1→4). (Libertadores não tem disputa de 3º lugar.)
  ...
  // Ties (nós do bracket), por rodada, chaveadas por (roundId, order). Preliminares +
  // oitavas: concretas (par por time, pelas pernas). Quartas/Semis/Final: placeholders
  // "a definir" — concreta substitui no mesmo (round,order) num re-seed. tieId/leg vão
  // nos jogos no loop de matches.
  const tieIdByEvent = new Map<string, string>();
  const legByEvent = new Map<string, number>();
  for (const slug of KO_SLUGS) {
    const roundId = roundIdBySlug.get(slug)!;
    const def = KNOCKOUT[slug];
    const roundEvents = koEvents.filter((e) => e.season!.slug === slug);
    if (roundEvents.length) {
      ...
      // Limpa placeholders antigos (esquema de ordem global anterior) e recria por-rodada.
      await prisma.tie.deleteMany({ where: { roundId, homeTeamId: null, awayTeamId: null } });
